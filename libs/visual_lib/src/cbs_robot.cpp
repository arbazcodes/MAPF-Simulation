...
CBS_Robot::CBS_Robot(glm::vec2 pos, float radius, glm::vec2 velocity, Texture2D sprite, glm::vec3 color)
...
void CBS_Robot::Rotate(float dt)
{
...
void CBS_Robot::Move(float dt, float unit_width, float unit_height, bool AllRobotsReached, bool AllRobotsRotated)
{
    if (!this->isMoving)
        return;
...
void CBS_Robot::Reset(glm::vec2 position, glm::vec2 velocity)
{
    this->CurrentPosition = position;
    this->Velocity = velocity;
}

void CBS_Robot::Draw(SpriteRenderer &renderer)
{
    renderer.DrawSprite(Sprite, CurrentPosition, glm::vec2(Radius * 2, Radius * 2), CurrentRotation, Color);
}

glm::vec2 CBS_Robot::GetPosition()
{
